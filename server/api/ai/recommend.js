        ...
        if (bizMap.size > 0) {
          ...
          const aboutCompletion = await groq.chat.completions.create({
            ...
            messages: [
              {
                role: 'system',
                content: `너는 증권 애널리스트다. 각 기업의 'DART 사업의 개요' 발췌를 읽고, 카드에 들어갈 아주 간결한 사업 소개를 쓴다.
규칙:
1. 길이: 1~2문장, 80자 내외로 최대한 압축. 카드 공간이 좁으니 군더더기·일반론·반복 제거.
2. ⚠️ 회사명으로 시작하지 말 것(카드에 종목명이 이미 크게 표시됨). 사업 내용부터 바로 시작.
   형식 예: "자동차 시트·차체 부품을 현대·기아에 공급하는 부품업체로, 1,000여종 양산 능력 보유." / "화재·자동차·장기보험을 취급하는 손해보험사."
3. 핵심 사업·주력 제품 + (있으면) 차별적 강점/기술 1개만. 나열 금지, 가장 중요한 것만.
4. 반드시 제공된 사업의 개요에 근거. 발췌에 없는 수치(점유율 등)·고객사·계약을 지어내지 말 것. 미확보 종목은 업종만 짧게.
5. 밸류에이션(PER/PBR/저평가)·주가·목표가 언급 금지. 오직 '사업 소개'만.
6. 정중한 존댓말 어미(~업체입니다/~합니다/~보유). 한글만. JSON 외 텍스트 금지.`,
              },
              {
                role: 'user',
...
JSON으로만 답하라(키는 종목코드, 값은 회사명 없이 1~2문장 압축 소개):
{"about":{"${missing[0]?.code || '코드'}":"○○하는 ○○업체로, ○○ 보유.", ...}}`,
              },
            ],
          });
